...
class MammoDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # print("data[self.label]  ",data[self.label])
        # label_mapping = {'BI-RADS 1': 0, 'BI-RADS 2': 1, 'BI-RADS 3': 2, 'BI-RADS 4': 3, 'BI-RADS 5': 4}
        # print("label_mappingdata[self.label]  ",label_mapping[data[self.label]])

        return {
            'x': img.unsqueeze(0),
            'y': torch.tensor(data[self.label], dtype=torch.long), #amal torch.tensor(label_mapping[data[self.label]], dtype=torch.long),
...
class MammoDataset_concept(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.dataset.lower() == 'upmc' and self.target_dataset.lower() == 'upmc':
            ...
            # study_id = str(self.df.iloc[idx]['patient_id']) #amal
            # print("study_id : ",study_id)
            # img_path = self.dir_path / f'{study_id}' / self.df.iloc[idx]['image_id']
            #/ f'{study_id}' amal
            img_path = self.dir_path  / self.df.iloc[idx]['patient_id'] / self.df.iloc[idx]['image_id'] #amal
            # print("img_path: ", img_path)
            # img_path = f'{img_path}.png'

        # img = cv2.imread(str(img_path), cv2.IMREAD_GRAYSCALE) 
        img = cv2.imread(str(img_path), cv2.IMREAD_COLOR) #amal
        ...
        elif self.args.model_type.lower() == "concept_classifier" and self.args.concept.lower() == 'clip_v1':
            y = self.df.iloc[idx]['CLIP_V1']
        elif self.args.model_type.lower() == "concept_classifier" and self.args.concept.lower() == 'mark_v1':
            y = self.df.iloc[idx]['MARK_V1']
        elif self.args.model_type.lower() == "concept_classifier" and self.args.concept.lower() == 'mole_v1':
            y = self.df.iloc[idx]['MOLE_V1']
        elif self.args.model_type.lower() == "concept_classifier" and self.args.concept.lower() == 'scar_v1':
            y = self.df.iloc[idx]['SCAR_V1']
        elif self.args.model_type.lower() == "concept_classifier" and self.args.concept.lower() == 'architectural_distortion':
            y = self.df.iloc[idx]['Architectural_Distortion']
        elif self.args.model_type.lower() == "concept_classifier" and self.args.concept.lower() == 'asymmetry':
            y = self.df.iloc[idx]['Asymmetry']
        elif self.args.model_type.lower() == "concept_classifier" and self.args.concept.lower() == 'focal_asymmetry':
            y = self.df.iloc[idx]['Focal_Asymmetry']
        elif self.args.model_type.lower() == "concept_classifier" and self.args.concept.lower() == 'global_asymmetry':
            y = self.df.iloc[idx]['Global_Asymmetry']
        elif self.args.model_type.lower() == "concept_classifier" and self.args.concept.lower() == 'mass':
            y = self.df.iloc[idx]['Mass']
        elif self.args.model_type.lower() == "concept_classifier" and self.args.concept.lower() == 'nipple_retraction':
            y = self.df.iloc[idx]['Nipple_Retraction']
        elif self.args.model_type.lower() == 'concept_classifier' and self.args.concept.lower() == 'skin_retraction':
            y = self.df.iloc[idx]['Skin_Retraction']
        elif self.args.model_type.lower() == 'concept_classifier' and self.args.concept.lower() == 'skin_thickening':
            y = self.df.iloc[idx]['Skin_Thickening']
        elif self.args.model_type.lower() == 'concept_classifier' and self.args.concept.lower() == 'suspicious_calcification':
            y = self.df.iloc[idx]['Suspicious_Calcification']
        elif self.args.model_type.lower() == 'concept_classifier' and self.args.concept.lower() == 'suspicious_lymph_node':
            y = self.df.iloc[idx]['Suspicious_Lymph_Node']
        elif self.args.model_type.lower() == 'concept_classifier' and self.args.concept.lower() == 'birads':
            y = self.df.iloc[idx]['birads']
        ...
        if self.dataset.lower == "vindr" or self.target_dataset.lower() == "vindr":
            boxes = [
            ...
            return {
                'x': img.unsqueeze(0),
                'y': y.astype(np.float32), #amal torch.tensor(label_mapping[y], dtype=torch.float),
                'img_path': str(img_path),
