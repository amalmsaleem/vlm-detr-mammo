...
import Detectors.detector.box_ops as box_ops
from Detectors.detector.misc import (NestedTensor, nested_tensor_from_tensor_list,
...
class DETR(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, aux_loss=False, num_channels=None):
...
#retinanet amal
        if self.training:
            img_batch, annotations = samples
        else:
            img_batch = samples

        # final_out, C3, C4, C5 = self.efficientnet(img_batch)
        _, C3, C4, C5 = self.backbone(img_batch)
        features = [C3, C4, C5]
...
def DETR_efficientnet(
        num_classes, model_type, device, clip_chk_pt=None, freeze_backbone="n", aux_loss=False):
    """[summary]
    ...
    if model_type == "clip_b5_upmc" or model_type == "clip_b5_upmc_vindr":
        ...
        model = DETR(
            num_classes=num_classes, backbone=efficientnet, num_queries =100, aux_loss=False, transformer=transformer, 
            num_channels=[176, 512, 2048]
        )
